...
def safe_serialize(obj):
    """Convert complex DALI metadata objects into JSON-friendly values."""
...
def process_dali_annotations_and_audio(anno_dir="./data/dali_raw_annotations", output_audio_dir="./data/raw_songs", max_tracks=None):
    """
    ...
    track_metadata_path = os.path.join(lines_dir, "track_metadata.jsonl")

    # Locate the .gz annotation files and ignore Windows zone identifier sidecar entries
    raw_files = glob.glob(os.path.join(anno_dir, "*.gz*"))
    gz_files = [p for p in raw_files if os.path.isfile(p) and os.path.basename(p).endswith(".gz")]
    if not gz_files:
    ...
    print(f"📦 Found {len(gz_files)} compressed DALI files in '{anno_dir}'.")
    if max_tracks is not None and max_tracks > 0:
        print(f"🎯 Targeting exact {max_tracks} successful track downloads.")

    # Clear previous execution manifests to start clean
    for path in (manifest_path, track_metadata_path):
    ...
    for idx, gz_path in enumerate(gz_files, 1):
        if max_tracks is not None and successful_tracks >= max_tracks:
            break

        track_id = os.path.splitext(os.path.basename(gz_path))[0]
        attempted_tracks += 1
        print(f"\n🔄 [{attempted_tracks}/{len(gz_files)}] Parsing file parameters for: {track_id}")
        ...
        try:
            ...
            # 2. Extract the embedded YouTube unique video string ID and language tag
            track_info = getattr(dali_object, 'info', {})
            audio_info = track_info.get('audio', {})
            youtube_id = audio_info.get('url', None)

            metadata_info = track_info.get('metadata', {}) or track_info.get('meta', {}) or {}
            language = metadata_info.get('language') or metadata_info.get('lang')
            language = str(language).strip().lower() if language else None
            if language not in ('english', 'eng', 'en'):
                print(f"  ⏩ Skipped: Non-English track detected ({language}).")
                track_metadata = {
                    "track_id": track_id,
                    "source_file": os.path.basename(gz_path),
                    "language": language,
                    "status": "non_english"
                }
                with open(track_metadata_path, "a", encoding="utf-8") as meta_out:
                    meta_out.write(json.dumps(safe_serialize(track_metadata)) + "\n")
                del dali_object
                gc.collect()
                continue
            ...
            successful_tracks += 1
            print(f"  ✅ Successfully mapped {len(raw_lines)} aligned sequences to manifest. Total successful tracks: {successful_tracks}")
    ...
    if max_tracks is not None and successful_tracks < max_tracks:
        print(f"⚠️ Only {successful_tracks} successful tracks were downloaded out of the requested {max_tracks}.")

if __name__ == "__main__":
    # Point this to the local DALI annotation folder and limit to ~60 tracks for fine tuning
    process_dali_annotations_and_audio(
        anno_dir="./data/annot_tismir",
        output_audio_dir="./data/fine_tune_songs",
        max_tracks=60
    )
